...
train_exp_dict = metrics.ExpDict('train.json').read(args.exp_dir)
...
for i, color in enumerate(colors):
    ...
    curves['UNet'] = np.array(
        train_multi_exp_dict[exp_name]['train_ious'])[:, i]
    alpha = 0.5
    gamma = 2.0
    exp_name = '{}_focal_True_alpha_{}_gamma_{}'.format(
        color, alpha, gamma)
    curves['Paw-Net'] = np.array(
        train_exp_dict[exp_name]['train_iou'])
    plot(axs[locs[i]], color_map[i], curves, args.step)
lg = plt.legend(list(curves.keys()), bbox_to_anchor=(
    1.04, 1.2), loc="upper left")
plt.savefig('figure5.png', bbox_extra_artists=(lg,), bbox_inches='tight')
plt.close()
